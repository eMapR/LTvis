...
function initializeInterface() {
    ...
    var earlyYearHTML = '<div class="slidecontainer"><input class="slider dataYear earlyYear" type="range" min="' + minYear + '" max="' + maxYear + '" value="' + minYear + '"><p class="sliderLabel">Year: <span class="earlyYearSpan">' + minYear + '</span></p></div>';

    var lateYearHTML = '<div class="slidecontainer"><input class="slider dataYear lateYear" type="range" min="' + minYear + '" max="' + maxYear + '" value="' + maxYear + '"><p class="sliderLabel">Year: <span class="lateYearSpan">' + maxYear + '</span></p></div>';
...
function makeOverlayLyrGroups() {
    ...
    for (var key in lyrGroups) {
        ...
        function markFeature(e) { // this is the style for that show if a tile is selected.
            ...
            if (!L.Browser.ie && !L.Browser.opera && !L.Browser.edge) { // this brings styling to the fr>
                layer.bringToFront();
            }
...
// Note: The detailed implementation of each function, especially those interacting with Leaflet or other libraries, has been omitted for brevity. The focus here is on organizing the script for better readability and maintainability.
